...
class AlignDlib:
    ...
    #: Landmark indices corresponding to the inner eyes and middle up-and-bottom lip.
    INNER_EYES_MIDDLE_LIP = [39, 42, 33, 51, 57]

    #: Landmark indices corresponding to the outer eyes and middle left and right mouth corners.
    OUTER_EYES_MIDDLE_MOUTH = [36, 45, 33, 48, 54]

    #: Landmark indices corresponding to the inner eyes and middle left and right mouth corners.
    INNER_EYES_MIDDLE_MOUTH = [39, 42, 33, 48, 54]

    #: Landmark indices corresponding to the outer eyes and middle up and bottom lips.
    OUTER_EYES_MIDDLE_LIP = [36, 45, 33, 51, 57]
    ...
    def align(self, imgDim, rgbImg, bb=None, pad=None, ts=None, sc=None,
              landmarks=None, landmarkIndices=INNER_EYES_MIDDLE_LIP,
              opencv_det=False, opencv_model="../model/opencv/cascade.xml",
              only_crop=False):
        r"""align(imgDim, rgbImg, bb=None, landmarks=None, landmarkIndices=INNER_EYES_MIDDLE_LIP)
...
def alignMain(args):
    ...
    if args.landmarks == 'outerEyesMiddleMouth':
        landmarkIndices = AlignDlib.OUTER_EYES_MIDDLE_MOUTH
    elif args.landmarks == 'innerEyesMiddleLip':
        landmarkIndices = AlignDlib.INNER_EYES_MIDDLE_LIP
    elif args.landmarks == 'outerEyesMiddleLip':
        landmarkIndices = AlignDlib.OUTER_EYES_MIDDLE_LIP
    elif args.landmarks == 'innerEyesMiddleMouth':
        landmarkIndices = AlignDlib.INNER_EYES_MIDDLE_MOUTH
    else:
...
if __name__ == '__main__':
    ...
    alignmentParser.add_argument('landmarks', type=str,
                                 choices=['outerEyesMiddleLip', 'innerEyesMiddleLip', 'outerEyesMiddleMouth', 'innerEyesMiddleMouth'],
    ...
    alignmentParser.add_argument('--ts', type=float, help="translation(,ts) the position of eyes downward so that..."
                                                        " we can reserve more area of forehead",
                                 default='30')
    alignmentParser.add_argument('--sc', type=float, help="Size from (middle) eyes downwards to (middle) mouth"
                                 default='32')
